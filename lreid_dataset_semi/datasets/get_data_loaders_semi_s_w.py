...
def get_data_purify( dataset, height, width, batch_size, workers, num_instances, Keep, Pseudo):    
    ...
    GTs=[x[-1] for x in dataset.train]
    if isinstance(Keep, torch.Tensor):
        Keep=Keep.cpu()
    else:
        Keep=torch.tensor(Keep).cpu()
    GTs=torch.tensor(GTs).cpu()
    Pseudo=torch.tensor(Pseudo).cpu()
    Correct=Pseudo==GTs
    Kept_num=Keep.float().sum()
    print(f"kept ratio {Kept_num/Keep.size(0)}\t kept precise {Correct[Keep].float().sum()/Kept_num}\t")
   
    return train_loader, init_loader


def get_data_purify_shrinkmatch( dataset, height, width, batch_size, workers, num_instances, Keep, Pseudo):    
    # create the clean data first
    ...
    train_transformer = transforms.Compose([
        ...
        transforms.RandomCrop(size=(height, width), padding=int(width*0.125), padding_mode='reflect'),
        transforms.ToTensor(),
        normalizer,
    ])
    
    train_transformer_strong = transforms.Compose([
        T.Resize((height, width), interpolation=3),
        transforms.RandomHorizontalFlip(),
        transforms.RandomCrop(size=(height, width), padding=int(width*0.125), padding_mode='reflect'),
    ...
    train_loader = IterLoader(
        DataLoader(Preprocessor(train_set, root=dataset.images_dir,transform=train_transformer, transform_strong=train_transformer_strong),
                   batch_size=batch_size, num_workers=workers, sampler=sampler,
                   shuffle=not rmgs_flag, pin_memory=True, drop_last=True), length=iters)
...
def build_data_loaders_semi(cfg, training_set, testing_only_set, select_num=500):
    # Create data loaders
    data_dir = cfg.data_dir
    height, width = (256, 128)
    training_loaders = [get_data_semi(name, data_dir, height, width, cfg.batch_size, cfg.workers,
                                 cfg.num_instances, select_num=select_num, label_ratio=cfg.label_ratio, training=True) for name in training_set]
