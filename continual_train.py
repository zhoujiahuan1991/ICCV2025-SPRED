...
from reid.trainer_semi import Trainer,eval_train,eval_train_acc
from torch.utils.tensorboard import SummaryWriter

from lreid_dataset_semi.datasets.get_data_loaders_semi import build_data_loaders_semi, get_data_purify
...
def extract_test_data_features(model,all_train_sets,all_test_sets,save_dir):
    # from reid.evaluators import extract_features_print
    all_sets=all_train_sets+all_test_sets
    for i in range(len(all_sets)):
        dataset, num_classes, train_loader, test_loader, init_loader, dataset_name = all_sets[i]
        print('Extracting features of {}'.format(dataset_name))
        features_all, labels_all, fnames_all, camids_all = extract_features(model, test_loader)
        # test_loader, dataset.query, dataset.gallery
        features={k:v for k,v in zip(fnames_all, features_all)}
        x = torch.cat([features[f[0]].unsqueeze(0) for f in dataset.query], 0)
        y = torch.cat([features[f[0]].unsqueeze(0) for f in dataset.gallery], 0)
        query=[x[1] for x in dataset.query]
        gallery = [x[1] for x in dataset.gallery]
        proto_type = {
            'query_features': x,
            'query_labels': query,
            'gallery_features': y,
            ...
            "features":features_all,
                'labels':labels_all,
                'file_names':fnames_all,
                'camids':camids_all
        }
        save_path = save_dir + '/{}.pt'.format(dataset_name)
        print("saving features of dataset {} to {}".format(dataset_name, save_path))
        torch.save(proto_type, save_path)

def extract_train_data_features(model,all_train_sets, save_dir): 
    save_dir=save_dir+'/train_new_feats'
    os.makedirs(save_dir, exist_ok=True) 
    model.eval()  

    
    for idx, data_info in enumerate(all_train_sets):
        dataset, num_classes, train_loader, test_loader, init_loader, dataset_name = data_info 
        ...
        with torch.no_grad():
            ...
            for i, (imgs, image_id, pids, cids, clean_pid) in enumerate(init_loader):
                ...
                for feature, pid, clean_id in zip(features, pids, clean_pid):
                    features_all.append(feature)
                    clean_labels_all.append(int(clean_id))
                    labels_all.append(pid)
        
        proto_type = {
            'features': features_all,
            'clean_labels': clean_labels_all,
            'data_labels': labels_all,            

        }
        save_path = save_dir + '/{}_new_train.pt'.format(dataset_name)
        print("saving features of dataset {} to {}".format(dataset_name, save_path))     
        torch.save(proto_type, save_path)      
...
def main_worker(args, cfg):  
    ...
    if args.test_folder:
        ...
        for i in range(args.n_model):
            ...
            # extract_test_data_features(model,all_train_sets,all_test_only_sets,args.test_folder)
            # extract_train_data_features(model,all_train_sets, args.test_folder)
...
def get_adaptive_alpha(args, model, model_old, all_train_sets, set_index,psedo_dist):
    ...
    Affin_new = get_normal_affinity(features_all_new, Norm=args.tau)
    Affin_old = get_normal_affinity(features_all_old, Norm=args.tau)
...
def train_dataset(cfg, args, all_train_sets, all_test_only_sets, set_index, model_list, out_channel, writer,logger_res=None):
    ...
    if set_index>=0:
        All_Labels=[x[1] for x in dataset.train]   
        Keep=[x[1]>=0 for x in dataset.train]     
        train_loader,init_loader_new=get_data_purify(dataset, height=256, width=128, batch_size=args.batch_size,
    ...
    for epoch in range(0, Epochs):       
        ...
        # record identity prediction acc
        if args.eval_acc: 
            _, _, _, _,acc=eval_train_acc(model_list[0],init_loader, add_num=add_num)        
            Accs.append(acc)
    ...
    if args.eval_acc: 
        torch.save(Accs, args.logs_dir+f'/{name}-acc.pt')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--AF_weight', default=4.0, type=float, help="anti-forgetting weight")    
    ...
    parser.add_argument('--tau', type=float,default=100, help="test during middle step")
    ...
    parser.add_argument('--eval_acc', action='store_true',default=False, help="testing the identity prediction accuracy during training")
    main()
