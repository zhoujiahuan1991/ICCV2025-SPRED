...
from lreid_dataset_semi.datasets.get_data_loaders_semi import  get_data_purify
...
class Trainer(object):
    def __init__(self,cfg,args, model_list, model_old_list, num_classes,origin_data,initial_labels,  writer=None,prototypes=None):
        ...
        self.origin_labels= torch.LongTensor(self.origin_labels)+num_classes-500
        ...
        self.pre_one_hot=scores_one_hot.clone() # 预测的分数
        ...
        self.clean_labels=[x[-1] for x in origin_data]  # 记录真实标签
    ...
    def obtain_cluster(self, init_loader, add_num, model_list,dataset_name=None,res_list=None,epoch=0):
        ...
        for m_id, model in enumerate(model_list):
            ...
            print(f"*********cluster number: {len(All_features)}\tcluster number: {num_cluster}")
    ...
    def train(self, epoch, data_loader_train,  optimizer_list, training_phase,
        ...
        for i in range(train_iters):    
            ...
            # indexes=domains # 域信息改成了图像在数据集中的顺序信息
            ...
            loss_ce1, loss_tp_1 = self.loss_fn(cls_outputs_1, s_features_1, targets, target_cam=None)
    ...
    def anti_forgetting(self, old_model,s_inputs,cls_outputs,s_features, targets,feat_final_layer, Keep=None):
        ...
        if self.args.spd:
            # feat_final_layer=feat_final_layer
            BS=feat_final_layer.size(0)
            feat_final_layer=feat_final_layer.reshape(BS,-1)
            feat_final_layer_old=feat_final_layer_old.reshape(BS,-1)
            GT = torch.mm(feat_final_layer, feat_final_layer.t())
            GT=F.normalize(GT,dim=1)
            GS = torch.mm(feat_final_layer_old, feat_final_layer_old.t())
            GS = F.normalize(GS, dim=1)
            Lsp=self.MSE(GT,GS)*3000
            loss+=Lsp
...
def eval_train_acc(model,init_loader, add_num=0):  
    model.eval()    
    Clean_IDS=torch.zeros(50000)
    Noisy_IDS=torch.zeros(50000)
    Clean_FLAG=torch.zeros(50000)    
    ...
    with torch.no_grad():
        for i, (imgs, image_id, pids, cids, clean_pid) in enumerate(init_loader):
            ...
            inputs, targets = inputs.cuda(), targets.cuda() 
            s_features_old, bn_feat_old, cls_outputs_old, feat_final_layer_old = model(inputs, get_all_feat=True)
            # s_features_old,  cls_outputs_old = model(inputs, get_all_feat=True)
            Count+=len(imgs)            
            features_all.append(s_features_old)
            for b in range(inputs.size(0)):               
                Clean_IDS[index[b]]=clean_pid[b]     
                Noisy_IDS[index[b]]=pids[b] 
                Clean_FLAG[index[b]]=clean_pid[b] ==  pids[b] 
    ...
    pre_ids=torch.softmax(All_logits,dim=1)[:,-500:].argmax(dim=-1)  # 预测的ID
    T_pre=pre_ids==(Clean_IDS)   # 找到预测正确的ID        
    acc=T_pre.float().sum().item()/len(Clean_FLAG)
    ...
    return Clean_IDS, Noisy_IDS, Clean_FLAG, All_logits,acc