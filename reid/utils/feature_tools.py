...
def extract_features(model, data_loader):
    ...
    with torch.no_grad():
        for i, (imgs, fnames, pids, cids, domains) in enumerate(data_loader):
            ...
            # except:
            #     imgs=imgs.to(model.module.device)
            imgs=imgs.cuda()
            features = model(imgs)
...
def initial_classifier(model, data_loader):
    ...
    pid_names=list(pid2features.keys())
    try:
        pid_names.remove(-1)
    except:
        pass
    class_centers = [torch.stack(pid2features[pid]).mean(0) for pid in sorted(pid_names)]
...
def euclidean_squared_distance(input1, input2):
    ...
    distmat = mat1 + mat2
    distmat.addmm_(mat1=input1.float(), mat2=input2.float().t(), beta=1, alpha=-2)
    return distmat

def initial_identity(model, data_loader, dataset,class_centers):
    pseudo_labels=[]
    features_all, labels_all, fnames_all, camids_all = extract_features(model, data_loader)
    features_all=torch.stack(features_all)
    features_all=F.normalize(features_all, dim=1).float().cuda()
    label_scores=features_all@class_centers.T
    # label_scores=1-euclidean_squared_distance(features_all, class_centers)/4
    pseudo_labels=pseudo_labels+torch.argmax(label_scores, dim=1).tolist()
    Initial_Labels=[]
    # for pseudo_label, fname in zip(pseudo_labels, fnames_all):
    #     Instance_info[fname]=pseudo_label
    
    
    for d, pseudo_label in zip(dataset.train, pseudo_labels):
        if d[1]==-1:
            Initial_Labels.append(pseudo_label)
        else:
            Initial_Labels.append(d[1])   
    if -1 in Initial_Labels:
        raise ValueError("-1 is in the labels")  
...
def extract_features_voro(model, data_loader, get_mean_feature=False):    
    ...
    if get_mean_feature:
        ...
        labels_named.sort()
        try:
            labels_named.remove(-1) # 去掉未标注的数据
        except:
            pass
        features_mean=[]        
        for x in labels_named:
